...
def solve(cxn, expected=None):
    ...
    get_key = log.progress('Getting key')
    key_mat = bytearray()
    while len(key_mat) < KEY_LEN:
        # do it in chunks to stay well clear of the KEY_LEN limit on input
        amt = min(1000, KEY_LEN - len(key_mat))
        ...
        key_chunk = bytes(ord('a') ^ b for b in enc_aaaa)
        key_mat.extend(key_chunk)
